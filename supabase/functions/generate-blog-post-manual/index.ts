...
Return ONLY a JSON object with:
{
  ...
  try {
    // Robust JSON extraction (handles ```json fences and stray text)
...
function getFeaturedImage(category: string): string {
  const images: Record<string, string> = {
    'AI News': 'photo-1677442136019-21780ecad995',
    'AI Services': 'photo-1620712943543-bcc4688e7485',
    'AI Thought Leadership': 'photo-1451187580459-43490279c0fa',
    'AI Tips': 'photo-1535378620166-273708d44e4c',
    'AI Consulting': 'photo-1485827404703-89b55fcc595e',
    'AI in the Workplace': 'photo-1552664730-d307ca884978'
  };
  
  const imageId = images[category] || images['AI News'];
  return `https://images.unsplash.com/${imageId}?w=1200&h=630&fit=crop&q=80`;
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { category: requestedCategory } = await req.json().catch(() => ({}));
    
    console.log("Starting manual blog post generation...");
    
    const supabase = createClient(SUPABASE_URL!, SUPABASE_SERVICE_ROLE_KEY!);
    
    // Use requested category or default to first one
    const category = categories.includes(requestedCategory) ? requestedCategory : categories[0];
    console.log(`Category: ${category}`);
    
    // Check recent topics (last 30 days)
    ...
    // PHASE 2: Trigger Editor Agent for post enhancement
    ...
    console.log("Manual blog post generation complete!");
